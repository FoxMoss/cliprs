...
static void quantize_row_q4_0(const float * restrict x, void * restrict y, int k) { quantize_row_q4_0_reference(x, y, k); }
...
static void quantize_row_q4_1(const float * restrict x, void * restrict y, int k) { quantize_row_q4_1_reference(x, y, k); }
...
static void quantize_row_q5_0(const float * restrict x, void * restrict y, int k) { quantize_row_q5_0_reference(x, y, k); }
...
static void quantize_row_q5_1(const float * restrict x, void * restrict y, int k) { quantize_row_q5_1_reference(x, y, k); }
...
static void quantize_row_q8_1(const float * restrict x, void * restrict vy, int k) {
    ...
    for (int i = 0; i < nb; i++) {
        ...
        y[i].s = d * (wasm_i32x4_extract_lane(accv, 0) + wasm_i32x4_extract_lane(accv, 1) + wasm_i32x4_extract_lane(accv, 2) +
...
#define GGML_F16_VEC_LOAD(p, i)                                                                                                \
    (i & 0x1) ? vec_extract_fp32_from_shorth(vec_xl(0, p - GGML_F16_EPR)) : vec_extract_fp32_from_shortl(vec_xl(0, p))
#define GGML_ENDIAN_BYTE(i) ((unsigned char *)&(uint16_t){1})[i]
#define GGML_F16_VEC_STORE(p, r, i)                                                                                            \
    if (i & 0x1)                                                                                                               \
    vec_xst(vec_pack_to_short_fp32(r[i - GGML_ENDIAN_BYTE(1)], r[i - GGML_ENDIAN_BYTE(0)]), 0, p - GGML_F16_EPR)
...
#define GGML_F32x4_REDUCE(res, x)                                                                                              \
    {                                                                                                                          \
        ...
        res = wasm_f32x4_extract_lane(x[0], 0) + wasm_f32x4_extract_lane(x[0], 1) + wasm_f32x4_extract_lane(x[0], 2) +         \
...
static void ggml_vec_dot_q5_0_q8_0(const int n, float * restrict s, const void * restrict vx, const void * restrict vy) {
...
#if defined(__ARM_FEATURE_DOTPROD)
        sumv0 = vmlaq_n_f32(
            sumv0, vcvtq_f32_s32(vaddq_s32(vdotq_s32(vdupq_n_s32(0), v0_0lf, v1_0l), vdotq_s32(vdupq_n_s32(0), v0_0hf, v1_0h))),
            GGML_FP16_TO_FP32(x0->d) * GGML_FP16_TO_FP32(y0->d));
        sumv1 = vmlaq_n_f32(
            sumv1, vcvtq_f32_s32(vaddq_s32(vdotq_s32(vdupq_n_s32(0), v0_1lf, v1_1l), vdotq_s32(vdupq_n_s32(0), v0_1hf, v1_1h))),
    ...
    *s = wasm_f32x4_extract_lane(sumv, 0) + wasm_f32x4_extract_lane(sumv, 1) + wasm_f32x4_extract_lane(sumv, 2) +
...
size_t ggml_nbytes_pad(const struct ggml_tensor * tensor) { return GGML_PAD(ggml_nbytes(tensor), GGML_MEM_ALIGN); }
...
float ggml_type_sizef(enum ggml_type type) { return ((float)(type_traits[type].type_size)) / type_traits[type].blck_size; }
...
const char * ggml_op_symbol(enum ggml_op op) { return GGML_OP_SYMBOL[op]; }
...
static struct ggml_object * ggml_new_object(struct ggml_context * ctx, enum ggml_object_type type, size_t size) {
    ...
    if (cur_end + size_needed + GGML_OBJECT_SIZE > ctx->mem_size) {
        GGML_PRINT("%s: not enough space in the context's memory pool (needed %zu, available %zu)\n", __func__,
                   cur_end + size_needed, ctx->mem_size);
        assert(false);
        return NULL;
    }
        ...
        if (ctx->scratch.data != NULL) {
            ...
            if (ctx->scratch.offs + data_size > ctx->scratch.size) {
                GGML_PRINT("%s: not enough space in the scratch memory pool (needed %zu, available %zu)\n", __func__,
                           ctx->scratch.offs + data_size, ctx->scratch.size);
                assert(false);
                return NULL;
            }
...
struct ggml_tensor * ggml_new_tensor_4d(struct ggml_context * ctx, enum ggml_type type, int64_t ne0, int64_t ne1, int64_t ne2,
                                        int64_t ne3) {
...
struct ggml_tensor * ggml_dup_inplace(struct ggml_context * ctx, struct ggml_tensor * a) { return ggml_dup_impl(ctx, a, true); }
...
static struct ggml_tensor * ggml_add1_impl(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b,
                                           bool inplace) {
...
static struct ggml_tensor * ggml_acc_impl(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b, size_t nb1,
...
struct ggml_tensor * ggml_acc(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b, size_t nb1, size_t nb2,
...
static struct ggml_tensor * ggml_sub_impl(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b,
                                          bool inplace) {
    GGML_ASSERT(ggml_are_same_shape(a, b));
...
static struct ggml_tensor * ggml_div_impl(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b,
                                          bool inplace) {
    GGML_ASSERT(ggml_are_same_shape(a, b));
...
struct ggml_tensor * ggml_sqr_inplace(struct ggml_context * ctx, struct ggml_tensor * a) { return ggml_sqr_impl(ctx, a, true); }
...
struct ggml_tensor * ggml_log_inplace(struct ggml_context * ctx, struct ggml_tensor * a) { return ggml_log_impl(ctx, a, true); }
...
static struct ggml_tensor * ggml_rms_norm_impl(struct ggml_context * ctx, struct ggml_tensor * a, float eps, bool inplace) {
    bool is_node = false;
...
static struct ggml_tensor * ggml_group_norm_impl(struct ggml_context * ctx, struct ggml_tensor * a, int n_groups,
                                                 bool inplace) {
...
static struct ggml_tensor * ggml_set_impl(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b, size_t nb1,
                                          size_t nb2, size_t nb3, size_t offset, bool inplace) {
...
struct ggml_tensor * ggml_set(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b, size_t nb1, size_t nb2,
                              size_t nb3, size_t offset) {
...
struct ggml_tensor * ggml_set_2d_inplace(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b, size_t nb1,
                                         size_t offset) {
    return ggml_set_impl(ctx, a, b, nb1, a->nb[2], a->nb[3], offset, false);
}
...
struct ggml_tensor * ggml_reshape_4d(struct ggml_context * ctx, struct ggml_tensor * a, int64_t ne0, int64_t ne1, int64_t ne2,
                                     int64_t ne3) {
    GGML_ASSERT(ggml_is_contiguous(a));
...
static struct ggml_tensor * ggml_view_impl(struct ggml_context * ctx, struct ggml_tensor * a, int n_dims, const int64_t * ne,
                                           size_t offset) {
...
static struct ggml_tensor * ggml_diag_mask_inf_impl(struct ggml_context * ctx, struct ggml_tensor * a, int n_past,
                                                    bool inplace) {
    bool is_node = false;
...
static struct ggml_tensor * ggml_diag_mask_zero_impl(struct ggml_context * ctx, struct ggml_tensor * a, int n_past,
                                                     bool inplace) {
    bool is_node = false;
...
static struct ggml_tensor * ggml_soft_max_back_impl(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b,
                                                    bool inplace) {
    bool is_node = false;
...
static struct ggml_tensor * ggml_rope_impl(struct ggml_context * ctx, struct ggml_tensor * a, int n_past, int n_dims, int mode,
                                           int n_ctx, float freq_base, float freq_scale, float xpos_base, bool xpos_down,
                                           bool inplace) {
...
struct ggml_tensor * ggml_rope_custom(struct ggml_context * ctx, struct ggml_tensor * a, int n_past, int n_dims, int mode,
                                      int n_ctx, float freq_base, float freq_scale) {
    return ggml_rope_impl(ctx, a, n_past, n_dims, mode, n_ctx, freq_base, freq_scale, 0.0f, false, false);
}

struct ggml_tensor * ggml_rope_custom_inplace(struct ggml_context * ctx, struct ggml_tensor * a, int n_past, int n_dims,
...
struct ggml_tensor * ggml_rope_xpos_inplace(struct ggml_context * ctx, struct ggml_tensor * a, int n_past, int n_dims,
...
GGML_API struct ggml_tensor * ggml_conv_1d(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b, int s0,
                                           int p0, int d0) {
...
struct ggml_tensor * ggml_conv_transpose_2d_p0(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b,
                                               int stride) {
...
static struct ggml_tensor * ggml_upscale_impl(struct ggml_context * ctx, struct ggml_tensor * a, int scale_factor) {
    ...
    struct ggml_tensor * result =
        ggml_new_tensor_4d(ctx, a->type, a->ne[0] * scale_factor, a->ne[1] * scale_factor, a->ne[2], a->ne[3]);
...
static struct ggml_tensor * ggml_add_rel_pos_impl(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * pw,
                                                  struct ggml_tensor * ph, bool inplace) {
...
struct ggml_tensor * ggml_add_rel_pos(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * pw,
                                      struct ggml_tensor * ph) {
    return ggml_add_rel_pos_impl(ctx, a, pw, ph, false);
}

struct ggml_tensor * ggml_add_rel_pos_inplace(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * pw,
                                              struct ggml_tensor * ph) {
...
static struct ggml_tensor * ggml_map_binary_impl_f32(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b,
...
static struct ggml_tensor * ggml_map_custom1_impl_f32(struct ggml_context * ctx, struct ggml_tensor * a,
...
static struct ggml_tensor * ggml_map_custom2_impl_f32(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b,
...
static struct ggml_tensor * ggml_map_custom3_impl_f32(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b,
...
static struct ggml_tensor * ggml_map_custom1_impl(struct ggml_context * ctx, struct ggml_tensor * a,
                                                  const ggml_custom1_op_t fun, int n_tasks, void * userdata, bool inplace) {
...
struct ggml_tensor * ggml_map_custom1(struct ggml_context * ctx, struct ggml_tensor * a, const ggml_custom1_op_t fun,
                                      int n_tasks, void * userdata) {
    return ggml_map_custom1_impl(ctx, a, fun, n_tasks, userdata, false);
}

struct ggml_tensor * ggml_map_custom1_inplace(struct ggml_context * ctx, struct ggml_tensor * a, const ggml_custom1_op_t fun,
                                              int n_tasks, void * userdata) {
...
static struct ggml_tensor * ggml_map_custom2_impl(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b,
                                                  const ggml_custom2_op_t fun, int n_tasks, void * userdata, bool inplace) {
    GGML_ASSERT(n_tasks == GGML_N_TASKS_MAX || n_tasks > 0);
...
struct ggml_tensor * ggml_map_custom2(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b,
                                      const ggml_custom2_op_t fun, int n_tasks, void * userdata) {
...
static struct ggml_tensor * ggml_map_custom3_impl(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b,
                                                  struct ggml_tensor * c, const ggml_custom3_op_t fun, int n_tasks,
...
struct ggml_tensor * ggml_map_custom3(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b,
                                      struct ggml_tensor * c, const ggml_custom3_op_t fun, int n_tasks, void * userdata) {
...
static void ggml_compute_forward_dup_same_cont(const struct ggml_compute_params * params, const struct ggml_tensor * src0,
    ...
    if (ie0 < ie1) {
        memcpy(((char *)dst->data + ie0 * nb0), ((char *)src0->data + ie0 * nb00), (ie1 - ie0) * ggml_type_size(src0->type));
    }
}
static void ggml_compute_forward_dup_f16(const struct ggml_compute_params * params, const struct ggml_tensor * src0,
    ...
    if (src0->type == dst->type && ne00 == ne0 && nb00 == ggml_type_size(src0->type) && nb0 == ggml_type_size(dst->type)) {
        // copy by rows
...
static void ggml_compute_forward_repeat_back_f32(const struct ggml_compute_params * params, const struct ggml_tensor * src0,
                                                 struct ggml_tensor * dst) {
    GGML_ASSERT(params->ith == 0);
        ...
        for (int i2 = ith; i2 < ne2; i2++) {
            if (i2 < ne02) { // src0
                ...
                for (int i1 = 0; i1 < ne1; i1++) {
                    for (int i0 = 0; i0 < ne0; i0++) {
                        const float * x =
                            (float *)((char *)src1->data + i0 * nb10 + i1 * nb11 + (i2 - ne02) * nb12 + i3 * nb13);
...
static void ggml_compute_forward_gelu_quick_f32(const struct ggml_compute_params * params, const struct ggml_tensor * src0,
                                                struct ggml_tensor * dst) {
    GGML_ASSERT(ggml_is_contiguous_except_dim_1(src0));
    GGML_ASSERT(ggml_is_contiguous_except_dim_1(dst));
    GGML_ASSERT(ggml_are_same_shape(src0, dst));
...
static void ggml_compute_forward_rms_norm_back_f32(const struct ggml_compute_params * params, const struct ggml_tensor * src0,
                                                   const struct ggml_tensor * src1, struct ggml_tensor * dst) {
        ...
        for (int64_t i02 = 0; i02 < ne02; i02++) {
            for (int64_t i01 = ith; i01 < ne01; i01 += nth) {
...

                {
                    ...
                    // grad[#00] = scale(grad(#10), #09) + scale(mul(#00, grad[#02]), 2.0)
                    // grad[#00] = scale(grad(#10), #09) + scale(mul(#00, repeat(-(sum(mul(grad[#10],#00)) * div(1,#07) *
                    // div(0.5, #08) * (1/N)), #02)), 2.0) grad[#00] = scale(grad(#10), #09) + scale(scale(#00,
                    // -(sum(mul(grad[#10],#00)) * div(1,#07) * div(0.5, #08) * (1/N))), 2.0) grad[#00] = scale(grad(#10), #09)
                    // + scale(#00, -(sum(mul(grad[#10],#00)) * div(1,#07) * div(1,#08) * (1/N))) grad[#00] = scale(grad(#10),
                    // #09) + scale(#00, sum(mul(grad[#10],#00)) * div(1,#07*#08) * (-1/N)) grad[#00] = scale(grad(#10), #09) +
                    ...
                    // scale(#00, sum(mul(grad[#10],#00)) * div(-1,rms*N*mean_eps)) grad[#00] = scale(grad(#10), #09) +
                    // scale(#00, sum(mul(grad[#10],#00)) * div(-1,rms*N*(sum_xx/N+eps))) grad[#00] = scale(grad(#10), #09) +
                    // scale(#00, sum(mul(grad[#10],#00)) * div(-1,rms*N*sum_xx+rms*N*eps)) grad[#00] = scale(dz, rrms) +
                    // scale(x, sum(mul(dz,x)) * div(-1,rms*N*mean_eps)) grad[#00] = scale(dz, rrms) + scale(x, sum_xdz *
                    ...
                    // (dz + x*sum_xdz * div(-1,rms*N*mean_eps)/rrms)*rrms a = (dz + x*sum_xdz *
                    // div(-1,rms*N*mean_eps)*rms)*rrms a = (dz + x*sum_xdz * div(-rms,rms*N*mean_eps))*rrms a = (dz + x*sum_xdz
                    // * div(-1,N*mean_eps))*rrms a = (dz + x*div(-sum_xdz,N*mean_eps))*rrms a = (dz +
                    // x*div(-mean_xdz,mean_eps))*rrms grad[#00] = scale(dz + scale(x, div(-mean_xdz,mean_eps)),rrms) grad[#00]
                    // = scale(dz + scale(x, -mean_xdz/mean_eps),rrms) dx = scale(dz + scale(x, -mean_xdz/mean_eps),rrms)
                }
...
static void ggml_compute_forward_group_norm_f32(const struct ggml_compute_params * params, const struct ggml_tensor * src0,
                                                struct ggml_tensor * dst) {
    GGML_ASSERT(ggml_are_same_shape(src0, dst));
...
static void ggml_compute_forward_get_rows_back_f32(const struct ggml_compute_params * params, const struct ggml_tensor * src0,
...
static void ggml_compute_forward_diag_mask_zero(const struct ggml_compute_params * params, const struct ggml_tensor * src0,
                                                struct ggml_tensor * dst) {
    switch (src0->type) {
    case GGML_TYPE_F32: {
...
static void ggml_compute_forward_soft_max_back_f32(const struct ggml_compute_params * params, const struct ggml_tensor * src0,
                                                   const struct ggml_tensor * src1, struct ggml_tensor * dst) {
    GGML_ASSERT(ggml_is_contiguous(src0));
            ...
            for (int64_t i1 = 0; i1 < ne1; i1++) {
                ...
                if (is_glm) {
                    ...
                    for (int64_t i0 = 0; i0 < ne0; i0 += 2) {
                        ...
                        float zeta =
                            xpos_base != 0.0f ? powf((i0 + 0.4f * ne0) / (1.4f * ne0), (n_past + i2) / xpos_base) : 1.0f;
                        if (xpos_down)
                            zeta = 1.0f / zeta;
                    ...
                    for (int64_t ib = 0; ib < ne0 / n_dims; ++ib) {
                        for (int64_t ic = 0; ic < n_dims; ic += 2) {
                            ...
                            ggml_fp16_t * dst_data =
                                (ggml_fp16_t *)((char *)dst->data + i3 * nb3 + i2 * nb2 + i1 * nb1 + i0 * nb0);
...
static void ggml_compute_forward_conv_1d_s1_ph_f32(const struct ggml_compute_params * params, const struct ggml_tensor * src0,
                                                   const struct ggml_tensor * src1, struct ggml_tensor * dst) {
...
static void ggml_compute_forward_conv_1d_s2_ph_f32(const struct ggml_compute_params * params, const struct ggml_tensor * src0,
                                                   const struct ggml_tensor * src1, struct ggml_tensor * dst) {
    GGML_ASSERT(src0->type == GGML_TYPE_F32);
    GGML_ASSERT(src1->type == GGML_TYPE_F32);
    GGML_ASSERT(dst->type == GGML_TYPE_F32);
...
static void ggml_compute_forward_conv_2d_f16_f32(const struct ggml_compute_params * params, const struct ggml_tensor * src0,
                                                 const struct ggml_tensor * src1, struct ggml_tensor * dst) {
    GGML_ASSERT(src0->type == GGML_TYPE_F16);
    GGML_ASSERT(src1->type == GGML_TYPE_F32);
    GGML_ASSERT(dst->type == GGML_TYPE_F32);
...
static void ggml_compute_forward_conv_transpose_2d(const struct ggml_compute_params * params, const struct ggml_tensor * src0,
                                                   const struct ggml_tensor * src1, struct ggml_tensor * dst) {
    GGML_ASSERT(src0->type == GGML_TYPE_F16);
    GGML_ASSERT(src1->type == GGML_TYPE_F32);
    GGML_ASSERT(dst->type == GGML_TYPE_F32);
        ...
        for (int i02 = ith; i02 < ne02; i02++) {
            for (int m = 0; m < dst->ne[1]; m++) {
                ...
                for (int n = 0; n < dst->ne[0]; n++) {
                    ...
                    float * y =
                        (float *)((char *)dst->data + n * dst->nb[0] + m * dst->nb[1] + i02 * dst->nb[2] + i03 * dst->nb[3]);
...
static void ggml_compute_forward_flash_attn_back_f32(const struct ggml_compute_params * params, const struct ggml_tensor * q,
...
static void ggml_compute_forward_win_unpart_f32(const struct ggml_compute_params * params, const struct ggml_tensor * src0,
                                                struct ggml_tensor * dst) {
    if (params->type == GGML_TASK_INIT || params->type == GGML_TASK_FINALIZE) {
        return;
    }
...
static void ggml_compute_forward_get_rel_pos_f16(const struct ggml_compute_params * params, const struct ggml_tensor * src0,
                                                 struct ggml_tensor * dst) {
    if (params->type == GGML_TASK_INIT || params->type == GGML_TASK_FINALIZE) {
        return;
    }
...
static void ggml_compute_forward_add_rel_pos_f32(const struct ggml_compute_params * params, const struct ggml_tensor * src0,
...
static void ggml_compute_forward_map_binary_f32(const struct ggml_compute_params * params, const struct ggml_tensor * src0,
                                                const struct ggml_tensor * src1, struct ggml_tensor * dst,
                                                const ggml_binary_op_f32_t fun) {
    assert(params->ith == 0);
    assert(ggml_are_same_shape(src0, src1) && ggml_are_same_shape(src0, dst));
...
static void ggml_compute_forward_cross_entropy_loss(const struct ggml_compute_params * params, const struct ggml_tensor * src0,
                                                    const struct ggml_tensor * src1, struct ggml_tensor * dst) {
    switch (src0->type) {
    case GGML_TYPE_F32: {
...
static void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor) {
    ...
    case GGML_OP_FLASH_FF: {
        ggml_compute_forward_flash_ff(params, tensor->src[0], tensor->src[1], tensor->src[2], tensor->src[3], tensor->src[4],
                                      tensor);
    } break;
    case GGML_OP_FLASH_ATTN_BACK: {
        ...
        GGML_ASSERT(t == 0 || t == 1);
        bool masked = t != 0;
        ggml_compute_forward_flash_attn_back(params, tensor->src[0], tensor->src[1], tensor->src[2], tensor->src[3], masked,
                                             tensor);
    } break;
...
static void ggml_compute_backward(struct ggml_context * ctx, struct ggml_tensor * tensor, bool inplace) {
    ...
    case GGML_OP_ADD1: {
        ...
        if (src1->grad) {
            src1->grad =
                ggml_add_impl(ctx, src1->grad, ggml_mean(ctx, tensor->grad), // TODO: should probably be sum instead of mean
                              inplace);
        }
    } break;
    case GGML_OP_ACC: {
        ...
        if (src1->grad) {
            ...
            struct ggml_tensor * tensor_grad_view = ggml_view_4d(ctx, tensor->grad, src1->grad->ne[0], src1->grad->ne[1],
    ...
    case GGML_OP_SQR: {
        if (src0->grad) {
            src0->grad = ggml_add_impl(ctx, src0->grad,
                                       ggml_scale(ctx, ggml_mul(ctx, src0, tensor->grad), ggml_new_f32(ctx, 2.0f)), inplace);
        }
    } break;
    case GGML_OP_SQRT: {
        if (src0->grad) {
            src0->grad = ggml_add_impl(ctx, src0->grad,
                                       ggml_scale(ctx, ggml_div(ctx, tensor->grad, tensor), ggml_new_f32(ctx, 0.5f)), inplace);
        }
    } break;
    case GGML_OP_LOG: {
        if (src0->grad) {
    ...
    case GGML_OP_RMS_NORM: {
        ...
        if (src0->grad) {
            ...
            src0->grad = ggml_add_impl(ctx, src0->grad, ggml_rms_norm_back(ctx, src0, tensor->grad, eps), inplace);
        }
    } break;
    case GGML_OP_RMS_NORM_BACK: {
        GGML_ASSERT(false); // TODO: not implemented
    ...
    case GGML_OP_SCALE: {
        ...
        if (src1->grad) {
            src1->grad = ggml_add_impl(ctx, src1->grad, ggml_sum(ctx, ggml_mul_impl(ctx, tensor->grad, src0, false)), inplace);
        }
    } break;
    case GGML_OP_SET: {
        const size_t nb1 = ((int32_t *)tensor->op_params)[0];
    ...
    case GGML_OP_DIAG_MASK_INF: {
        ...
        if (src0->grad) {
            ...
            src0->grad = ggml_add_impl(ctx, src0->grad, ggml_diag_mask_zero_impl(ctx, tensor->grad, n_past, false), inplace);
        }
    } break;
    case GGML_OP_DIAG_MASK_ZERO: {
        // necessary for llama
    ...
    case GGML_OP_ROPE: {
        ...
        if (src0->grad) {
            ...
            src0->grad = ggml_add_impl(
                ctx, src0->grad,
                ggml_rope_back(ctx, tensor->grad, n_past, n_dims, mode, n_ctx, freq_base, freq_scale, xpos_base, xpos_down),
                inplace);
        }
    } break;
    case GGML_OP_ROPE_BACK: {
        if (src0->grad) {
            ...
            src0->grad = ggml_add_impl(ctx, src0->grad,
                                       ggml_rope_impl(ctx, tensor->grad, n_past, n_dims, mode, n_ctx, freq_base, freq_scale,
    ...
    case GGML_OP_FLASH_ATTN: {
        ...
        if (src0->grad) {
            ...
            case 4: {
                grad_q = ggml_view_4d(ctx, flash_grad, src0->ne[0], src0->ne[1], src0->ne[2], src0->ne[3], nb0 * src0->ne[0],
        ...
        if (src1->grad) {
            ...
            case 4: {
                grad_k = ggml_view_4d(ctx, flash_grad, src1->ne[0], src1->ne[1], src1->ne[2], src1->ne[3], nb0 * src1->ne[0],
        ...
        if (opt0->grad) {
            ...
            case 4: {
                grad_v = ggml_view_4d(ctx, flash_grad, opt0->ne[0], opt0->ne[1], opt0->ne[2], opt0->ne[3], nb0 * opt0->ne[0],
    ...
    case GGML_OP_UNARY: {
        ...
        case GGML_UNARY_OP_RELU: {
            if (src0->grad) {
                src0->grad = ggml_add_impl(ctx, src0->grad, ggml_mul(ctx, ggml_step(ctx, src0), tensor->grad), inplace);
            }
        } break;
        case GGML_UNARY_OP_GELU: {
            GGML_ASSERT(false); // TODO: not implemented
    ...
    case GGML_OP_CROSS_ENTROPY_LOSS: {
        if (src0->grad) {
            src0->grad = ggml_add_impl(ctx, src0->grad, ggml_cross_entropy_loss_back(ctx, src0, src1, tensor->grad), inplace);
        }
    } break;
    case GGML_OP_CROSS_ENTROPY_LOSS_BACK: {
        GGML_ASSERT(false); // not supported
...
size_t ggml_graph_overhead(void) { return GGML_OBJECT_SIZE + GGML_PAD(GGML_GRAPH_SIZE, GGML_MEM_ALIGN); }
...
static void ggml_graph_export_leaf(const struct ggml_tensor * tensor, FILE * fout) {
    ...
    fprintf(fout, "%-6s %-12s %8d %" PRId64 " %" PRId64 " %" PRId64 " %" PRId64 " %16zu %16zu %16zu %16zu %16p %32s\n",
            ggml_type_name(tensor->type), ggml_op_name(tensor->op), tensor->n_dims, ne[0], ne[1], ne[2], ne[3], nb[0], nb[1],
            nb[2], nb[3], tensor->data, tensor->name);
}
...
static void ggml_graph_export_node(const struct ggml_tensor * tensor, const char * arg, FILE * fout) {
    ...
    fprintf(fout, "%-6s %-6s %-12s %8d %" PRId64 " %" PRId64 " %" PRId64 " %" PRId64 " %16zu %16zu %16zu %16zu %16p %32s\n",
            arg, ggml_type_name(tensor->type), ggml_op_name(tensor->op), tensor->n_dims, ne[0], ne[1], ne[2], ne[3], nb[0],
...
void ggml_graph_export(const struct ggml_cgraph * cgraph, const char * fname) {
    ...
    // print
    {
        ...
        fprintf(fout, "%-6s %-6s %-12s %8s %8s %8s %8s %8s %16s %16s %16s %16s %8s %16s %16s\n", "ARG", "TYPE", "OP", "NDIMS",
                "NE0", "NE1", "NE2", "NE3", "NB0", "NB1", "NB2", "NB3", "NTASKS", "DATA", "NAME");
...
static enum ggml_opt_result ggml_opt_adam(struct ggml_context * ctx, struct ggml_opt_context * opt,
                                          ...
                                          struct ggml_cgraph * gb, ggml_opt_callback callback, void * callback_data) {
...
static enum ggml_opt_result linesearch_backtracking(const struct ggml_opt_params * params, int nx, float * x, float * fx,
                                                    float * g, float * d, float * step, const float * xp,
                                                    struct ggml_tensor * f, struct ggml_cgraph * gf, struct ggml_cgraph * gb,
                                                    struct ggml_cplan * cplan, const int np, struct ggml_tensor * ps[],
...
enum ggml_opt_result ggml_opt_resume(struct ggml_context * ctx, struct ggml_opt_context * opt, struct ggml_tensor * f) {
    ...
    struct ggml_tensor * gfbuf = ggml_new_tensor_1d(ctx, GGML_TYPE_I32,
                                                    sizeof(struct ggml_cgraph) / ggml_type_size(GGML_TYPE_I32) +
                                                        (sizeof(struct ggml_cgraph) % ggml_type_size(GGML_TYPE_I32) ? 1 : 0));
    struct ggml_tensor * gbbuf = ggml_new_tensor_1d(ctx, GGML_TYPE_I32,
                                                    sizeof(struct ggml_cgraph) / ggml_type_size(GGML_TYPE_I32) +
                                                        (sizeof(struct ggml_cgraph) % ggml_type_size(GGML_TYPE_I32) ? 1 : 0));
...
enum ggml_opt_result ggml_opt_resume_g(struct ggml_context * ctx, struct ggml_opt_context * opt, struct ggml_tensor * f,
                                       struct ggml_cgraph * gf, struct ggml_cgraph * gb, ggml_opt_callback callback,
                                       void * callback_data) {
...
struct gguf_context * gguf_init_from_file(const char * fname, struct gguf_init_params params) {
        ...
        for (uint32_t i = 0; i < ctx->header.n_kv; ++i) {
            ...
            case GGUF_TYPE_ARRAY: {
                ...
                case GGUF_TYPE_BOOL: {
                    ...
                    ok = ok &&
                         gguf_fread_el(file, kv->value.arr.data, kv->value.arr.n * GGUF_TYPE_SIZE[kv->value.arr.type], &offset);
                } break;
                case GGUF_TYPE_STRING: {
    ...
    // compute the total size of the data section, taking into account the alignment
    {
        ...
        for (uint32_t i = 0; i < ctx->header.n_tensors; ++i) {
            ...
            const int64_t ne = (int64_t)info->ne[0] * (int64_t)info->ne[1] * (int64_t)info->ne[2] * (int64_t)info->ne[3];
    ...
    if (params.ctx != NULL) {
        ...
        const size_t mem_size = params.no_alloc ? (ctx->header.n_tensors) * ggml_tensor_overhead()
...
const char * gguf_get_key(const struct gguf_context * ctx, int i) { return ctx->kv[i].key.data; }

enum gguf_type gguf_get_kv_type(const struct gguf_context * ctx, int i) { return ctx->kv[i].type; }

enum gguf_type gguf_get_arr_type(const struct gguf_context * ctx, int i) { return ctx->kv[i].value.arr.type; }

const void * gguf_get_arr_data(const struct gguf_context * ctx, int i) { return ctx->kv[i].value.arr.data; }
...
int gguf_get_arr_n(const struct gguf_context * ctx, int i) { return ctx->kv[i].value.arr.n; }

uint8_t gguf_get_val_u8(const struct gguf_context * ctx, int i) { return ctx->kv[i].value.uint8; }

int8_t gguf_get_val_i8(const struct gguf_context * ctx, int i) { return ctx->kv[i].value.int8; }

uint16_t gguf_get_val_u16(const struct gguf_context * ctx, int i) { return ctx->kv[i].value.uint16; }

int16_t gguf_get_val_i16(const struct gguf_context * ctx, int i) { return ctx->kv[i].value.int16; }

uint32_t gguf_get_val_u32(const struct gguf_context * ctx, int i) { return ctx->kv[i].value.uint32; }

int32_t gguf_get_val_i32(const struct gguf_context * ctx, int i) { return ctx->kv[i].value.int32; }

float gguf_get_val_f32(const struct gguf_context * ctx, int i) { return ctx->kv[i].value.float32; }

uint64_t gguf_get_val_u64(const struct gguf_context * ctx, int i) { return ctx->kv[i].value.uint64; }

int64_t gguf_get_val_i64(const struct gguf_context * ctx, int i) { return ctx->kv[i].value.int64; }

double gguf_get_val_f64(const struct gguf_context * ctx, int i) { return ctx->kv[i].value.float64; }

bool gguf_get_val_bool(const struct gguf_context * ctx, int i) { return ctx->kv[i].value.bool_; }

const char * gguf_get_val_str(const struct gguf_context * ctx, int i) { return ctx->kv[i].value.str.data; }
    ...
    for (uint32_t i = 0; i < src->header.n_kv; i++) {
        ...
        case GGUF_TYPE_ARRAY: {
            if (src->kv[i].value.arr.type == GGUF_TYPE_STRING) {
                ...
                gguf_set_arr_data(ctx, src->kv[i].key.data, src->kv[i].value.arr.type, src->kv[i].value.arr.data,
